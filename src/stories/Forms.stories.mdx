...
pass as <b>props</b> whenever we use it. It contains another custom component [TextInput](/?path=/docs/textinput--text-input)
which controls each input. Based on the schema provided, there is email is validating
if the email input data doesn't follow the email standards. The <b>
  Phone Number
</b> input accepts only 16 numbers as an input. The user needs to add his/hers <b>
  country code
</b> before its phone number.
...
the inputs. There are validation in every input except Other Information input which
is optional. <b>Postal code</b> input accepts only 5 characters otherwise it will
error out.
...
error out. `Expiration date` input has an error boundary. If the date provided
belongs to the past it will error out as well. Lastly, `CVV` input errors out if
